...
flowchart TB
    user["사용자 / 데모 PC"]
    dns["Bind9 DNS<br/>Route 53 역할 모의"]
    vip["HAProxy + Keepalived VIP<br/>On-prem 진입점"]
    ingress["ingress-nginx<br/>NodePort"]
    app["FlaskApp Pods<br/>flaskapp-prod"]
    db["MariaDB VM<br/>On-prem Primary DB"]
    ...
    vpn["pfSense Site-to-Site VPN"]
    dms["AWS DMS<br/>Full Load + CDC"]
    rds["AWS RDS<br/>DR DB"]
    s3["AWS S3<br/>애플리케이션 파일 저장소"]
    eks["AWS EKS + ALB<br/>DR 전환 시 활성화"]

    user --> dns --> vip --> ingress --> app --> db
    github --> ecr --> helm --> argocd --> app
    app --> s3
    db --> vpn --> dms --> rds
    prom --> app
    app --> loki --> holmes
